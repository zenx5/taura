...
import { generateAction, menuConfig, help, fromFile, renderListModels, selectGlobal, selectModel, setKey } from "./actions.js";
...
(async function () {
    ...
    if (ACTIONS.GENERATE)
        ...
        await help();
    else if (ACTIONS.FILE)
        await fromFile(getNameFile(arg));
...
function getNameFile(arg) {
    if (arg.includes(flags.fileName)) {
        ...
        return arg[index + 1] ?? process.env.AI_MODEL_FILENAME_DEFAULT;
    }
    return process.env.AI_MODEL_FILENAME_DEFAULT;
}
